def build_prompt(shots = ('joy', 'sadness'), prompt_index = 0, emotions_list = None):
    sample_shots = [
    ...
    # Use custom emotion list if provided, otherwise use default
    if emotions_list is None:
        emotions_list = ['anger', 'boredom', 'disgust', 'fear', 'guilt', 'joy', 'pride', 'relief', 'sadness', 'shame', 'surprise', 'trust', 'neutral']
    
    emotion_list_str = "Consider this list of emotions: " + ", ".join(emotions_list) + ". "
    

    prompt0 = f"What are the inferred emotions in the following contexts?"
    prompt1 = emotion_list_str + prompt0
    prompt2 = ''
    ...
    prompt8 = emotion_list_str + prompt4
    prompt9 = emotion_list_str + prompt5
    # TODO play with the sample shots less prompts (one shot, 2 shot, up to 13 shot)

    prompt = [prompt0, prompt1, prompt2, prompt3, prompt4, prompt5, prompt6, prompt7, prompt8, prompt9][prompt_index]
    #
